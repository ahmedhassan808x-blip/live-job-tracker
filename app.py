...
def background_loop():
    """Refresh on startup, then every REFRESH_SECONDS."""
    while True:
        try:
            refresh_all()
        except Exception as e:
...
def health():
    return jsonify({"ok": True, "has_key": bool(RAPIDAPI_KEY), "status": CACHE["status"]})


@app.route("/")
def home():
    # Serve the front-end if it's in the same folder
...
# Start the background refresher as soon as the app boots
